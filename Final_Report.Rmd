...
**We also explored any differences in the proportions between Katie_27 and Katie_43 receiving a favorable reply. We ran a tw-proportion z-test resulting in the following:**
...
**With a p-value greater than $0.05$, we conclude that the proportion of favorable replies between the two Katies' is not significantly different.**
...
Prior to our experiment, we did some rough power calculations.  However, after tabulating results and output, it appears that our experiment suffered from a lack of power, but only an incredibly drastic change in observations or effect would have garnered a statistically sgnificant result.  Examing our calculated effect, we would need an n of `r round(as.numeric(power_n['n']),0)` (or 32,434) for our results to be statistically significant.  Keeping the number of actual observations, we would have needed a measured treatment effect of `r round(as.numeric(power_d['d']),2)`, nearly twenty times our observed effect.  This lends evidence to the lack of effect we observed and suggests an experiment of a signficantly larger scale would be required to identify any real effect.

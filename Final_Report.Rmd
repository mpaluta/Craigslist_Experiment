...
It's important to call out any OLS assumptions that may have been violated. We addressed our decision to use linear regression with a binary outcome above, and we can confirm that random samplying occurred. Through exploratory data analysis, we confirmed there was no multi-collinearity across covariates, and if there was, only one covariate was chosen such that the models did not contain collinear independent variables.

Several covariates, such as "housing type" and "date/time" are not linear. 
...
```{r}
durbinWatsonTest(mod.1)
```


**---> Below is list I pulled for reference only**

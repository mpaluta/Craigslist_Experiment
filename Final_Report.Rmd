...
< Through our research, we find no effect>

< This is important because it signals a lack of disparity in housing opportunity for women aged 27 and 43 in Indianapolis, perhaps suggesting that age is not a barrier to find a roommate. >
...
## Methodology
<TODO>
- Fill in specifics now that experiment is determined
- Add flow diagram
- Largely just clean/format, most of the content is there.
...
We will try two methods of testing for treatment effect. The first method we will use is regression. Regression has the advantage of utilizing covariate information to add precision to estimates of our treatment effect. As favorable replies are a binary response, logistic regression is the most appropriate way to model probabilities and odds of a response. Within regression, we will use four specifications.

Our first model contains all key covariates <explain why we chose (completeness, preservation of degrees of freedom.)>
Our second removed posting type and price - neither was significant and likely correlated, removing does not impact results
Third contains only significant variable and treatment
Fourth is treatment only
...
model.1 = glm(fave_reply ~ treatment+log(list_price)+list_type+log(hours_post_to_email)+sent_is_weekend, data=d, family = binomial(link="logit"))
summary(model.1)

# model 2: removing list type and price (rationale - maybe correlated with weekend sent)
model.2 = glm(fave_reply ~ treatment+log(hours_post_to_email)+sent_is_weekend, data=d, family = binomial(link="logit"))
summary(model.2)

# model 3: only treatment and hours_post_to_email (the only significant covariate)
model.3 = glm(fave_reply ~ treatment+log(hours_post_to_email), data=d, family = binomial(link="logit"))
summary(model.3)

# model 4: treatment only
model.4 = glm(fave_reply ~ treatment, data=d, family = binomial(link="logit"))
summary(model.4)
...
- ensure stargazer working appropriately
...
The second way we will analyze this data is simply a two-proportion z-test. We cannot use covariates for this test, but we can 


### Randomization Inference

First, let's get our ATEs:
...
table(ate_vec<ate)
```
This shows that, out of 10000 random instantiations, 3949 show an effect stronger than our calculated ATE, implying a p-value of 0.3949, a non-statistically significant result.  This is consistent with our linear modeling results.
...
```{r}
pwr.t.test(d=0.022, sig.level=0.05, power=.9)
pwr.t.test(n=113, sig.level=0.05, power=.9)
```
<TODO>
- Add narrative
